...
import foundry.common.json.minify
...
private object JsonLiteralSerializer : KSerializer<JsonLiteral> {
  ...
  override fun deserialize(decoder: Decoder): JsonLiteral {
    ...
    if (result !is JsonLiteral) {
      throw SerializationException(
        "Unexpected JSON element, expected JsonLiteral, had ${result::class}. JSON input: ${result.toString().minify(-1)}"
      )
    }
    return result
  }
