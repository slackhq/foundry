...
```bash
./gradlew :tools:cli:build -Dorg.gradle.isolated-projects=false
```
...
2. Run `./gradlew publishToMavenLocal -Dorg.gradle.isolated-projects=false` to publish the current version to your local maven repository.
