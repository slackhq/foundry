...
/**
 * Configures [KotlinCompile] tasks with the given [action] but _ignores_ KaptGenerateStubsTask and
 * KSP types as they inherit arguments from standard compilation tasks via and applying arguments to
 * them could result in duplicates.
 *
 * See
...
internal fun TaskContainer.configureKotlinCompilationTask(
  ...
  withType(KotlinCompilationTask::class.java).configureEach {
    ...
    if (includeKaptGenerateStubsTask || !this.name.startsWith("kapt")) {
      if (includeKspTask || !this.name.startsWith("ksp")) {
        action()
      }
    }
  }
