...
interface CircuitComponent {
  ...
  fun createCircuitInjectAnnotation(
...
class CircuitPresenter(
  ...
  override fun generate(packageName: String, className: String): FileSpec {
    ...
    val constructorBuilder =
      if (!addAssistedInjection) {
        FunSpec.constructorBuilder()
          .addParameter(ParameterSpec.builder("screen", screenClass).build())
          ...
          .apply {
            if (addAssistedInjection) {
              ...
              circuitInjectScope?.let {
                addType(createFactoryBuilder(screenClass, it, className).build())
              }
            } else {
              circuitInjectScope?.let {
                addAnnotation(createCircuitInjectAnnotation(screenClass, circuitInjectScope))
              }
            }
          }
      }
  ...
  private fun createFactoryBuilder(
    ...
    return TypeSpec.funInterfaceBuilder("Factory")
      ...
      .addAnnotation(createCircuitInjectAnnotation(screenClass, injectScope))
      .addFunction(
