...
class GradleDependencyManager {
  fun addMissingGradleDependency(project: Project, directory: String) {
    ...
    val gradlePath = findNearestProjectDirRecursive(Paths.get(repoRoot), Paths.get(directory), mutableMapOf()) ?: return
  ...
  fun addCircuitImport(gradleBuildModel: GradleBuildModel?,
                               project: Project
  ): PsiElement? {
    val scripts = PsiTreeUtil.findChildrenOfType(gradleBuildModel?.psiFile, KtScriptInitializer::class.java)
    val psiFactory = KtPsiFactory(project)
    ...
    val slackBlock = """
        ...
        """.trimIndent()
    gradleBuildModel?.pluginsPsiElement?.let { return addExpressionToBlock(it, slackBlock, psiFactory, project ) }
    return null
  }
  ...
  private fun addExpressionToBlock(block: PsiElement, expressionText: String, psiFactory: KtPsiFactory, project: Project): PsiElement {
    WriteCommandAction.runWriteCommandAction(project) {
      block.add(psiFactory.createNewLine())
