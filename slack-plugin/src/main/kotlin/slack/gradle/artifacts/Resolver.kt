...
internal class Resolver<T : Named>(
  ...
  fun artifactView(): Provider<Set<File>> = internal.flatMap { configuration ->
    configuration.incoming
      .artifacts
      .resolvedArtifacts
      .map { resolvedArtifactResults -> resolvedArtifactResults.mapTo(mutableSetOf()) { it.file } }
  }
  ...
  fun addSubprojectDependencies(slackProperties: SlackProperties) {
    project.dependencies.apply {
      for (subproject in project.subprojects) {
        ...
        add(
          declarable.name,
          project.project(subproject.path)
        )
      }
    }
  }
